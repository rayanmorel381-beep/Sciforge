...
| Module | Changes |
|---|---|
| Architecture | Refactored monorepo → Cargo workspace (10 crates) — `sciforge-lib`, `sciforge-hub`, `sciforge-core`, `sciforge-automotive`, `sciforge-aeronaval`, `sciforge-rail`, `sciforge-heavy`, `sciforge-parser`, `sciforge-benchmark`, `sciforge` (facade) |
| Core | Extracted `components/` from `sciforge-core` — pièces physiques concrètes séparées des primitives transverses |
| Facade | `src/lib.rs` thin re-export facade; features `lib`, `hub`, `aeronaval`, `automotive`, `core`, `components`, `heavy`, `full` |
| [Constants](ChangeLog/constants.md) | Removed `SPEED_OF_LIGHT_KM_S` alias (duplicate of `C_KM_S`) from `constants::maths::approximation`; 6 new particle-physics constants (`G_F`, `ALPHA_S`, `MUON_MASS`, `TAU_MASS`, `NEUTRINO_MASS_UPPER`, `R_INF`); full planetary constants Mercury → Neptune |
