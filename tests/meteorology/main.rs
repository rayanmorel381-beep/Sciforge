use sciforge::benchmark::engine::{BenchmarkMetrics, bench};
...
fn generate_output_files() {
    let (dir, test_count) = export_outputs("meteorology", "SciForge Meteorology Test");
    assert_artifacts("meteorology", &dir, test_count);
}
