...
const LAYOUT: LayoutKind = LayoutKind::Inline;
...
pub fn character_for(d: &Displacement) -> SoundCharacter {
    let bs_ratio = d.bore_mm / d.stroke_mm;
    let liters = d.liters();
    if liters < 0.9 && bs_ratio > 1.0 {
        SoundCharacter::Buzzy
    } else if bs_ratio > 1.05 {
        SoundCharacter::Revvy
    } else if liters > 1.3 && bs_ratio < 0.85 {
        SoundCharacter::Boomy
    } else if liters > 1.2 {
        SoundCharacter::Torquey
    } else {
...
fn voice_loudness(exhaust: ExhaustVoice) -> VoiceLoudness {
    match exhaust {
        ...
        ExhaustVoice::TriYBurble => VoiceLoudness::Sporty,
    }
}
