...
const LAYOUT: LayoutKind = LayoutKind::Vr;
...
pub fn character_for(d: &Displacement) -> SoundCharacter {
    if d.liters() >= 2.4 { SoundCharacter::VR5Sport }
    else if d.liters() >= 2.2 { SoundCharacter::VR5Burble }
    else { SoundCharacter::VR5Smooth }
}

pub fn fundamental_hz(rpm: u32) -> f64 { ac::fundamental_hz(rpm, CYLINDERS) }
pub fn idle_rpm_for(d: &Displacement) -> u32 { ac::idle_rpm(d.liters(), LAYOUT) }
...
fn voice_loudness(exhaust: ExhaustVoice) -> VoiceLoudness {
    match exhaust {
        ...
        ExhaustVoice::VR5Burble => VoiceLoudness::VrBurble,
        ExhaustVoice::VR5Sport => VoiceLoudness::VrBurble,
