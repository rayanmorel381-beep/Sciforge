...
pub fn character_for(d: &Displacement) -> SoundCharacter {
    let liters = d.liters();
    if liters >= 8.2 {
        SoundCharacter::W16Thunder
    } else if liters >= 7.5 {
        SoundCharacter::W16BugattiHowl
    } else {
        SoundCharacter::W16Symphony
    }
}
...
fn voice_loudness(exhaust: ExhaustVoice) -> VoiceLoudness {
    match exhaust {
        ...
        ExhaustVoice::W16Thunder => VoiceLoudness::Race,
        ExhaustVoice::W16BugattiHowl => VoiceLoudness::Race,
        ExhaustVoice::W16Symphony => VoiceLoudness::W8Smooth,
...
pub fn natural(d: &Displacement, exhaust: ExhaustVoice) -> EngineSound {
    ...
    EngineSound {
        ...
        crackle_pop: matches!(exhaust, ExhaustVoice::Aggressive | ExhaustVoice::Race | ExhaustVoice::W16Thunder | ExhaustVoice::W16BugattiHowl),
        turbo_spool_hz: None,
...
pub fn turbo(d: &Displacement, exhaust: ExhaustVoice, count: u8, anti_lag: bool) -> EngineSound {
    let multi = count >= 2;
    ...
    let mut exh = ac::exhaust_loudness_db(liters, voice_loudness(exhaust), egt_k, redline, CYLINDERS, exhaust_port_d_m) - 3.0;
    if anti_lag { exh += 4.0; }
    base.exhaust_loudness_db = exh;
