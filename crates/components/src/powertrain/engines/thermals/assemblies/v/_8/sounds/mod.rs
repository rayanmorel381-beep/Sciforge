...
const LAYOUT: LayoutKind = LayoutKind::V;
const HEAD_MATERIAL: &std::sync::LazyLock<sciforge_core::materials::Material> = &CGI_400;
const INTERCOOLED: bool = true;
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SoundCharacter {
    CrossPlaneRumble,
    FlatPlaneScream,
    Burble,
    MuscleRumble,
...
pub fn character_for(d: &Displacement) -> SoundCharacter {
    if d.flat_plane {
        SoundCharacter::FlatPlaneScream
    } else if d.liters() >= 6.0 {
        SoundCharacter::MuscleRumble
    } else if d.liters() >= 4.5 {
        SoundCharacter::Aggressive
    } else if d.liters() >= 4.0 {
        SoundCharacter::CrossPlaneRumble
    } else {
        SoundCharacter::Burble
    }
}
...
fn voice_loudness(exhaust: ExhaustVoice) -> VoiceLoudness {
    match exhaust {
        ...
        ExhaustVoice::V8Burble => VoiceLoudness::CrossPlane,
        ExhaustVoice::MuscleRumble => VoiceLoudness::CrossPlane,
        ExhaustVoice::FlatPlaneScream => VoiceLoudness::FlatPlane,
    }
}
...
pub fn natural(d: &Displacement, exhaust: ExhaustVoice) -> EngineSound {
    ...
    EngineSound {
        ...
        crackle_pop: matches!(exhaust, ExhaustVoice::Aggressive | ExhaustVoice::Race | ExhaustVoice::FlatPlaneScream),
        turbo_spool_hz: None,
...
pub fn turbo(d: &Displacement, exhaust: ExhaustVoice, count: u8) -> EngineSound {
    let multi = count >= 2;
    let boost = if multi { BoostKind::TwinTurbo } else { BoostKind::Turbo };
    let liters = d.liters();
    let redline = redline_rpm_for(d);
    let exhaust_port_d_m = (d.bore_mm * 0.35) / 1000.0;
    let intake_port_d_m = (d.bore_mm * 0.40) / 1000.0;
    ...
    base.induction = if multi { InductionSound::TwinTurboWhistle } else { InductionSound::TurboWhistle };
    ...
    base.induction_loudness_db = ac::induction_loudness_db(liters, redline, CYLINDERS, intake_port_d_m, charge_k) + 8.0 + (count as f64 - 1.0) * 1.5;
    base.harmonic_richness = ac::harmonic_richness(HEAD_MATERIAL, egt_k, liters) + 0.10;
    let inducer_mm = 30.0 + liters * 12.0 / count as f64;
    base.turbo_spool_hz = Some(ac::turbo_spool_hz(base.redline_rpm, inducer_mm, multi));
