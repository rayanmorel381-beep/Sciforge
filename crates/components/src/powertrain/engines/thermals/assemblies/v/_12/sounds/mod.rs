...
pub fn character_for(d: &Displacement) -> SoundCharacter {
    let liters = d.liters();
    if (6.4..=6.6).contains(&liters) {
        SoundCharacter::V12Wail
    } else if liters >= 6.0 {
        SoundCharacter::V12Symphony
    } else {
...
fn voice_loudness(exhaust: ExhaustVoice) -> VoiceLoudness {
    match exhaust {
        ...
        ExhaustVoice::V12Wail => VoiceLoudness::CrossPlane,
        ExhaustVoice::V12Symphony => VoiceLoudness::LuxuryHum,
...
pub fn natural(d: &Displacement, exhaust: ExhaustVoice) -> EngineSound {
    ...
    EngineSound {
        ...
        crackle_pop: matches!(exhaust, ExhaustVoice::Aggressive | ExhaustVoice::Race),
        turbo_spool_hz: None,
