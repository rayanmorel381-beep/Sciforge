...
const LAYOUT: LayoutKind = LayoutKind::Flat;
...
pub fn character_for(d: &Displacement) -> SoundCharacter {
    ...
    let liters = d.liters();
    if matches!((liters, bs_ratio), (l, _) if l < 2.0) {
        SoundCharacter::Rumble
    } else if bs_ratio > 1.2 {
...
fn voice_loudness(exhaust: ExhaustVoice) -> VoiceLoudness {
    match exhaust {
        ...
        ExhaustVoice::BoxerRumble => VoiceLoudness::BoxerRumble,
    }
}
