...
fn build(disp_l: f64, layout: u8) -> WetClutch {
    ...
    let diams: [f64; 8] = [130.0, 140.0, 150.0, 160.0, 170.0, 180.0, 190.0, 200.0];
    let counts: [u8; 8] = [6u8, 7, 8, 9, 10, 11, 12, 13];
    let tier = tier_for(disp_l);
    let max_torque_nm = (torques[tier] * 1.8 * layout_mul(layout) * 100.0).round() / 100.0;
    WetClutch::atv(counts[tier], diams[tier], max_torque_nm)
}
