...
impl ManualGearbox {
    ...
    fn new(expected: usize, forward_teeth: Vec<(u32, u32)>, reverse_teeth: (u32, u32)) -> Self {
        ...
        let module_mm: f64 = 2.5;
        let face_width_mm: f64 = 22.0;
        let endurance_pa: f64 = 480.0e6;
        let safety: f64 = 1.5;
        ...
        let efficiency = 0.985_f64.powi(forward_teeth.len() as i32);
    ...
    pub fn tick(
        ...
        self.wear_factor += (pitting_rate * cycles / 1.0e9 + overload * 0.01 + overheat * 0.001 + self.oil_contamination * 0.001) * dt_s;
        ...
        if self.oil_temp_k_runtime > 453.0 && !self.failures.contains(&GearboxFailureMode::BearingSeized) {
        ...
        if self.hours_run > 3000.0 && !self.failures.contains(&GearboxFailureMode::SynchronizerWorn) {
            self.failures.push(GearboxFailureMode::SynchronizerWorn);
        }
    }
