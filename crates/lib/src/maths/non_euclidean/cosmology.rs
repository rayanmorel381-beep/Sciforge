use crate::constants::{C_KM_S, CMB_RECOMBINATION_REDSHIFT};
...
pub fn comoving_distance(h0: f64, omega_m: f64, omega_lambda: f64, z: f64) -> f64 {
    let n = 1000;
    let h = z / n as f64;
    let f = |zi: f64| C_KM_S / hubble_parameter(h0, omega_m, omega_lambda, zi);
...
pub fn horizon_distance(h0: f64, omega_m: f64, z: f64) -> f64 {
    2.0 * C_KM_S / (h0 * omega_m.sqrt() * (1.0 + z).sqrt())
}
