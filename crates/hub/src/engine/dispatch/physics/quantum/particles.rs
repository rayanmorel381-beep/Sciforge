...
pub(crate) fn dispatch(func: &str, p: &Params) -> HubResult<RunOutput> {
    match func {
        ...
        "planck_energy" => Ok(RunOutput::Scalar(phys::quantum::particle::planck_energy())),
        "photon_energy" => Ok(RunOutput::Scalar(
            phys::quantum::particle::photon_energy(get_f(p, "frequency")?),
        )),
        "fine_structure_constant" => Ok(RunOutput::Scalar(
            phys::quantum::particle::fine_structure_constant(),
        )),
        "compton_time" => Ok(RunOutput::Scalar(
            phys::quantum::particle::compton_time(get_f(p, "mass")?),
        )),
