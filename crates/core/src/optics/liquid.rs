...
impl Liquid {
    pub fn refractive_index(&self) -> Option<f64> {
        refractive_index::by_formula_phase(self.formula, "liquid").map(|r| r.n_d)
    }
    ...
    pub fn numerical_aperture(&self, half_angle_rad: f64) -> Option<f64> {
        self.refractive_index()
            .map(|n| sf_refr::numerical_aperture(n, half_angle_rad))
    }
}
